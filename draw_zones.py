...
def resolve_source(source_path: str) -> Optional[np.ndarray]:
    if source_path.endswith('.m3u8'):
        # Si es un video en vivo, usa av para leer el stream
        import av
        container = av.open(source_path)
        frame_generator = (frame.to_ndarray(format="bgr24") for frame in container.decode(video=0))
        frame = next(frame_generator)
        return frame
    else:
        # Si es un video local o una imagen, usa la función actual
